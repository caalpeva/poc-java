...
public class TestLambdas {
	...
	public void test_function() {
		...
		IntUnaryOperator cuadrado = o -> o * o;
	...
	public void test_funcion_2() {
		...
		LongBinaryOperator menor = (a, b) -> Math.min(a, b);
	...
	public void test_extras() {
		...
		Predicate<Persona> isNullLastName = p -> p.getApellido2() == null; 
		
		assertTrue(isNullLastName.test(personaSinSegundoApellido));
		assertFalse(isNullLastName.test(personaConSegundoApellido));
		assertFalse(isNullLastName.test(personaNoPariente));
		
		// Una funcion que nos diga si dos personas son parientes: para nosotros parientes
		// son personas con el mismo primer apellido
		BiPredicate<Persona, Persona> areParients = (p1, p2) -> p1.getApellido1().equals(p2.getApellido1());
		
		assertTrue(areParients.test(personaSinSegundoApellido, personaConSegundoApellido));
		assertFalse(areParients.test(personaSinSegundoApellido, personaNoPariente));
		assertFalse(areParients.test(personaConSegundoApellido, personaNoPariente));
